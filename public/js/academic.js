...
async function loadStudents() {
  ...
  try {
    ...
    let rows = students.map(s => `
      <tr>
        <td><strong>${s.fullName}</strong><br><small class="text-muted">${s.grNo || 'N/A'}</small></td>
        <td>${s.fatherName}</td>
...
async function loadClasses() {
  ...
  try {
      ...
      <div class="col-md-3 mb-3">
        <div class="card shadow-sm">
          <div class="card-body text-center">
            ...
            <button class="btn btn-sm btn-outline-danger" onclick="deleteClass('${c._id}', '${c.className}')"><i class="bi bi-trash"></i> Delete</button>
          </div>
        </div>
      </div>
    `).join('');
    ...
    main.innerHTML = `
      <button class="btn btn-primary mb-3" onclick="showClassModal()"><i class="bi bi-plus-circle"></i> Add Class</button>
